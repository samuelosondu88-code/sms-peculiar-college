...
if ($action === 'download' && $selectedStudent && $selectedSession && $selectedTerm && $selectedClass) {
    ...
    $student = $stmt->fetch();
    if (!$student) { redirect('/teacher/results/pdf.php'); }

    $summary = getStudentTermSummary($db, $selectedStudent, $selectedSession, $selectedTerm);
    if ($summary['subject_count'] === 0) { redirect('/teacher/results/pdf.php?error=no_scores'); }
        ...
        <form method="GET" class="row g-3">
            <div class="col-md-3">
                ...
                <select name="session_id" class="form-select" onchange="this.form.submit()">
                    <?php $sessions = $db->query("SELECT id,session_name FROM academic_sessions ORDER BY start_date DESC")->fetchAll(); ?>
            ...
            <div class="col-md-3">
                ...
                <select name="term_id" class="form-select">
                    ...
                    <?php $tStmt = $db->prepare("SELECT id,term_name FROM terms WHERE session_id=? ORDER BY id");
                    $tStmt->execute([$selectedSession]); $terms = $tStmt->fetchAll(); ?>
            ...
            <div class="col-md-3">
                ...
                <select name="student_id" class="form-select">
                    ...
                    <option value="<?=$s['id']?>" <?=$selectedStudent===$s['id']?'selected':''?>><?=sanitizeInput($s['last_name'].', '.$s['first_name'])?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-12">
...
<div class="card">
    ...
    <div class="card-body p-0">
        <table class="table table-hover mb-0">
            <thead><tr><th>#</th><th>Student</th><th>Admission No</th><th>Action</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="?session_id=<?=$selectedSession?>&term_id=<?=$selectedTerm?>&class_id=<?=$selectedClass?>&student_id=<?=$s['id']?>&action=download" class="btn btn-sm btn-outline-danger"><i class="fas fa-file-pdf me-1"></i>PDF</a></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
