...
$classes = $db->prepare("
    SELECT c.id, c.name, c.section, c.code, c.capacity,
        (SELECT COUNT(*) FROM student_subject ss JOIN subject_allocations sa ON ss.subject_id = sa.subject_id WHERE sa.class_id = c.id AND sa.teacher_id = ?) as student_count,
    ...
    ORDER BY c.name
");
$classes->execute([$teacherId, $teacherId, $teacherId]);
$myClasses = $classes->fetchAll();

$totalStudents = $db->prepare("SELECT COUNT(DISTINCT ss.student_id) FROM student_subject ss JOIN subject_allocations sa ON ss.subject_id = sa.subject_id WHERE sa.teacher_id = ?");
    ...
    <div class="col-md-4">
        <div class="card h-100">
            <div class="card-body">
                ...
                <p class="small text-muted mb-1">Code: <?= sanitizeInput($c['code'] ?? 'N/A') ?></p>
