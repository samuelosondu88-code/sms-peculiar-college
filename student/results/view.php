...
$psycho = $db->prepare("SELECT * FROM psychomotor_assessments WHERE student_id = ? AND session_id = ? AND term_id = ?");
$psycho->execute([$studentId, $sessionId, $termId]);
$psychomotor = $psycho->fetch();

$affect = $db->prepare("SELECT * FROM affective_assessments WHERE student_id = ? AND session_id = ? AND term_id = ?");
$affect->execute([$studentId, $sessionId, $termId]);
$affective = $affect->fetch();

$comm = $db->prepare("SELECT * FROM result_comments WHERE student_id = ? AND session_id = ? AND term_id = ?");
$comm->execute([$studentId, $sessionId, $termId]);
$comments = $comm->fetch();

$ins = $db->prepare("SELECT * FROM academic_insights WHERE student_id = ? AND session_id = ? AND term_id = ?");
$ins->execute([$studentId, $sessionId, $termId]);
$insights = $ins->fetch();

$prom = $db->prepare("SELECT * FROM promotion_results WHERE student_id = ? AND session_id = ?");
$prom->execute([$studentId, $sessionId]);
$promotion = $prom->fetch();
...
<div class="d-flex justify-content-between align-items-center mb-3 no-print">
    <h4 class="fw-bold mb-0">
        ...
        <small class="text-muted fs-6"><?= sanitizeInput($sessionName . ' - ' . $termName) ?></small>
    </h4>
    <div>
    ...
    <div class="card-body">
        ...
        <div class="row mb-4">
            <div class="col-md-8">
                <table class="table table-sm table-borderless">
                    <tr>
                        <td style="width: 140px;"><strong>Student Name:</strong></td>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-8">
                <table class="table table-sm table-borderless mb-0">
                    <tr>
                        <td style="width: 160px;"><strong>Total Marks Obtained:</strong></td>
                    ...
                    <tr>
                        <td><strong>Student's Average:</strong></td>
                        <td><strong><?= number_format($summary['average'], 1) ?>%</strong></td>
                    </tr>
                    <tr>
                        <td><strong>Class Position:</strong></td>
                        <td><strong><?= $position ? $position . niceOrdinal($position) : '-' ?></strong></td>
                    </tr>
                    <tr>
                        <td><strong>Overall Grade:</strong></td>
                        <td>
                            <span class="badge bg-<?= $summary['overall_grade'] === 'A' ? 'success' : ($summary['overall_grade'] === 'F' ? 'danger' : 'primary') ?> fs-6">
                    ...
                    <tr>
                        ...
                        <td><?= $summary['pass_count'] ?> Passed / <?= $summary['fail_count'] ?> Failed</td>
                    </tr>
                </table>
            </div>
            <div class="col-md-4">
                    ...
                    <div class="card-body p-0">
                        <table class="table table-sm table-bordered mb-0">
                            <thead class="table-light">
                                <tr><th>Trait</th><th class="text-center">Rating</th></tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td><?= $label ?></td>
                                    <td class="text-center"><span class="badge bg-secondary"><?= $psychomotor[$key] ?? 'B' ?></span></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div class="card-body p-0">
                        <table class="table table-sm table-bordered mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?= $label ?></td>
                                    <td class="text-center"><span class="badge bg-secondary"><?= $affective[$key] ?? 'B' ?></span></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(sanitizeInput($comments['class_teacher_remark'])) ?></p>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(sanitizeInput($comments['principal_remark'])) ?></p>
                    </div>
                </div>
            </div>
            <?php endif; ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <ul class="small mb-0 ps-3">
                            <?php foreach (explode("\n", $insights['strengths']) as $s): if (trim($s)): ?>
                            <li><?= sanitizeInput($s) ?></li>
                    ...
                    <div class="col-md-4">
                        ...
                        <ul class="small mb-0 ps-3">
                            <?php foreach (explode("\n", $insights['weaknesses']) as $s): if (trim($s)): ?>
                            <li><?= sanitizeInput($s) ?></li>
                            <?php endif; endforeach; ?>
                        </ul>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <ul class="small mb-0 ps-3">
                            <?php foreach (explode("\n", $insights['recommendations']) as $s): if (trim($s)): ?>
                            <li><?= sanitizeInput($s) ?></li>
                            <?php endif; endforeach; ?>
                        </ul>
                    </div>
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-4 text-center">
                ...
                <?php
                $promBadge = match ($promotion['promotion_status']) {
                    'promoted' => 'bg-success',
                ...
                <p class="small mt-1 mb-0"><?= sanitizeInput($promotion['remark']) ?></p>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            <div class="col-md-4 text-end">
                <p class="small text-muted mb-0">Generated on: <?= date('d M, Y') ?></p>
                <p class="small text-muted mb-0">Peculiar International College</p>
            </div>
        </div>
    </div>
</div>
...
<style>
@media print {
    .no-print, #sidebar-wrapper, .navbar, footer { display: none !important; }
